...
export class QuestionnaireComponent implements OnInit {
  ...
  constructor(private m_UserService: UserService, private m_SnackBar: MatSnackBar,
     private m_Router: Router, private m_QuestionnaireService: QuestionnaireService) { }

  ngOnInit() {
    this.m_UserService.m_Data$.pipe(
      take(1),
      switchMap(data => {
        return this.m_QuestionnaireService.getAnsweredQuestionnaireByUserId(data?.id).pipe(
          switchMap(questionnaire => {
            ...
            if (data?.sex === 'MALE') {
              return this.m_QuestionnaireService.getMaleQuestions().pipe(tap(questions => {
              ...
              return this.m_QuestionnaireService.getFemaleQuestions().pipe(tap(questions => {
                this.dataSource = questions;
                for (let question of questions) {
                  this.form.addControl(question.id, new UntypedFormControl(false));
                }
  ...
  onSubmit() {
    ...
    this.m_QuestionnaireService.submitAnswers(obj)
    .pipe(catchError(res => {
